...
def main():
    ...
    while processes or Q1 or Q2 or Q3 or cpu_state or io_state or finished_process:
        ...
        # Assume separate lines for each finished process
        for process in finished_process[:]:
            done_process = finished_process.pop(0)
            print(done_process, "DONE")

        # Check for demoted processes
        # Assume separate lines for each finished process
        for process in finished_process[:]:
            demote_process = demoted_process.pop(0)
            print(demote_process, "DEMOTED")
        ...
        if cpu_state:
            ...
            if current_burst["type"] == "CPU":
                # Decrement burst duration at the end of the time step
                should_decrement = True
                should_remove = False
                should_append = False
                should_demote_q1 = False
                should_demote_q2 = False

                # Check if CPU burst is complete
                if current_burst["duration"] == 1:
                    should_remove = True

                # Check if quantum expires
                elif cpu_state["queue_level"] == "Q1":
                    if cpu_state["allotment_counter"] == 1:
                        should_demote_q1 = True
                    elif cpu_state["quantum_counter"] == 1:
                        should_append = True
                elif cpu_state["queue_level"] == "Q2" and cpu_state["quantum_counter"] == 1:
                    should_demote_q2 = True
        ...
        if cpu_state:
            ...
            if should_decrement:
                ...
                if cpu_state["queue_level"] == "Q1":
                    cpu_state["allotment_counter"] -= 1 # Decrement the process's time allotment counter
            if should_remove:
                cpu_state["bursts"].pop(0)  # Remove completed burst
                if not cpu_state["bursts"]:  # Process is done
                    cpu_state["finish_time"] = current_time  # Mark finish time
